...
function init()
  ...
  g_keyboard.bindKeyDown('Ctrl+Shift+J', jump)
  
  ProtocolGame.registerExtendedOpcode(108, removeMapFlag)
  minimapWindow:setup()
...
function jump()
   g_game.getLocalPlayer():jump(5, 200)
end
...
function miniMapClean()
    minimapWidget:cleanFlags()
    minimapWidget:cleanFlags()
    minimapWidget:cleanFlags()
    minimapWidget:cleanFlags()
end

function removeMapFlag(protocol, opcode, buffer)
for x = -20,20 do
for y = -20,20 do
  local player = g_game.getLocalPlayer()
  if not player then return end
local pos = {x = player:getPosition().x+(x),y = player:getPosition().y+(y),z = player:getPosition().z}
