...
bool LuaInterface::loadDirectory(const std::string& dir, Npc* npc/* = NULL*/, bool recursively/* = false*/)
{
	...
	for(boost::filesystem::directory_iterator it(dir), end; it != end; ++it)
	{
		...
		{
			if(recursively && !loadDirectory((it->path() / s).string(), npc, recursively))
				return false;
		}
...
void LuaInterface::registerFunctions()
{
	...
	//AQUI REGISTRAR AS NOVAS FUN��ES DO NTO (Lembrando que, em TFS 0.4 LuaScripInterface � LuaInterface apenas).
	...
	lua_register(m_luaState, "doCreateMonsterNick", LuaInterface::luaDoCreateMonsterNick);
	...
	lua_register(m_luaState, "getCreatureNickRealName", LuaInterface::luaGetCreatureNickRealName);
...
int32_t LuaInterface::luaGetCreatureNickRealName(lua_State* L)
{
	//getCreatureNickRealName(cid)
	ScriptEnviroment* env = getEnv();
...
int32_t LuaInterface::luaDoCreateMonsterNick(lua_State* L)
{
	//doCreateMonsterNick(monster, nick, pos)
	ScriptEnviroment* env = getEnv();
	PositionEx pos;
