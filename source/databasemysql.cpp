	...
	m_timeoutTask(0)
{
	...
	// Desabilitar SSL/TLS completamente (MariaDB Connector/C 3.4+ habilita SSL por padrao)
	// 1. Variavel de ambiente - deve ser definida ANTES de mysql_real_connect
	putenv((char*)"MARIADB_TLS_DISABLE_PEER_VERIFICATION=1");

	// 2. Desabilitar enforcement e verificacao via mysql_options
	my_bool ssl_mode_off = 0;
	mysql_options(&m_handle, MYSQL_OPT_SSL_ENFORCE, &ssl_mode_off);
	mysql_options(&m_handle, MYSQL_OPT_SSL_VERIFY_SERVER_CERT, &ssl_mode_off);

	// 3. Chamar mysql_ssl_set com NULLs para limpar qualquer config SSL residual
	mysql_ssl_set(&m_handle, NULL, NULL, NULL, NULL, NULL);

	// 4. Forcar o campo use_ssl da struct MYSQL para false
	m_handle.options.use_ssl = 0;
