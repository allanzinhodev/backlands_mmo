...
function init()
  ...
  if protocol then
  protocol:sendExtendedOpcode(27, "hehe")
  end
  connect(LocalPlayer, {
  ...
  ProtocolGame.registerExtendedOpcode(27, getNinja) 
...
function refresh()
  ...
  skillsWindow:setContentMaximumHeight(550)
end
...
function toggle()
skillsWindow:setSize({width = 380, height = 550})
...
function getNinja(protocol, opcode, buffer)
...
function getNinjaInfo(protocol, opcode, buffer)
 ...
 local village = msg:getData()
  ...
  local basespeed = msg:getData()
  player:setBaseSpeed(basespeed)
  
  local movspeed = msg:getData()
...
function onSkillChange(localPlayer, id, level, percent)
  ...
  setSkillPercent(skills2[id], percent, tr('You have %s percent to go', 100 - percent))
