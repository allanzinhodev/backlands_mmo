...
local function setProtocol(protocol)
  if not protocolBox then return end
  local protocolStr = tostring(protocol)
  local hasOption = false
  for _, opt in ipairs(protocolBox.options) do
    if opt.text == protocolStr then
      hasOption = true
      break
    end
  end
  if not hasOption then
    protocolBox:addOption(protocolStr)
  end
  protocolBox:setCurrentOption(protocolStr)
end
...
function EnterGame.init()
  ...
  EnterGame.setUniqueServer('127.0.0.1', 7171, 854, 200, 190)
end
